  ...
  if (
    ...
    [...foreground.children].forEach((child, index) => {
      ...
      if (foreground.childElementCount === 2 && index === 1)
        child.className = 'tablet-up desktop-up';
    });
  }
  return foreground;
}
...
function decorateBlockIconArea(el) {
  ...
  headings.forEach((h) => {
    ...
    if (hPrevElem?.childElementCount) {
      ...
      if (picCount === hPrevElem.childElementCount)
        hPrevElem.classList.add('icon-area');
    }
  });
}
function singleLinkClickPod(el) {
  if (el.querySelectorAll('a').length === 1) {
    const links = el.querySelectorAll('a');
    console.log('single link click pod');
...
export default function init(el) {
  ...
  if (el.classList.contains('full-width'))
    helperClasses.push('max-width-8-desktop', 'center', 'xxl-spacing');
