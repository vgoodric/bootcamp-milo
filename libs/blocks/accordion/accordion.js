...
function displayMedia(displayArea, el, dd, i, expanded) {
  ...
  [...mediaCollection[id]].forEach((mediaCollectionItem, idx, total) => {
    mediaCollectionItem.classList.remove('expanded');

    total.forEach((element, index) => {
      const trigger = document.querySelector(
        `#accordion-${id}-trigger-${index + 1}`
      );
...
function handleClick(el, dd, num) {
  ...
  if (closestEditorial)
    displayMedia(
      closestEditorial.querySelector('.accordion-media'),
      el,
      dd,
      num,
      expanded
...
function createItem(accordion, id, heading, num, edit) {
  ...
  const analyticsString = `open-${num}--${processTrackingLabels(
    heading.textContent
  )}`;
  ...
  const dtHtml = hTag
    ? createTag(hTag.tagName, { class: 'accordion-heading' }, button)
    : button;
  const dt = createTag('dt', dtAttrs, dtHtml);
  const dd = createTag(
    'dd',
    { role: 'region', 'aria-labelledby': triggerId, id: panelId, hidden: true },
    panel
  );
...
export default function init(el) {
  ...
  const accordion = createTag('dl', {
    class: 'accordion',
    id: `accordion-${id}`,
    role: 'presentation',
  });
  const accordionMedia = createTag('div', {
    class: 'accordion-media',
    id: `accordion-media-${id}`,
  });
  ...
  if (isEditorial) {
    ...
    [...editorialMedia].map((media, idx, collection) =>
  ...
  const maxWidthClass = Array.from(el.classList).find((style) =>
